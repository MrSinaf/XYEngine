...
internal class SplashScreen : Scene
{
	...
	private static void LoadDefaultAssets()
	{
		...
		AssetManager.LoadEmbeddedAsset<Shader>("shaders.ui.shadxy", new ShaderConfig(material =>
		{
			...
			material.SetProperty("margin", new Region(Vector2.zero));
			material.SetProperty("marginScale", 1F);
		}, (material, name, obj) =>
		{
		...
		UIPrefab.Add<Panel>(null, Panel.DefaultStyle);
		UIPrefab.Add<Button>(null, Button.DefaultStyle);
		UIPrefab.Add<Label>(null, Label.DefaultStyle);
		UIPrefab.Add<ProgressBar>(null, ProgressBar.DefaultStyle);
	}
