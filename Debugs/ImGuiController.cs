...
public class ImGuiController
{
	...
	private void CreateDeviceResources()
	{
		...
		GCommandQueue.Enqueue(() => io.Fonts.SetTexID((IntPtr)fontTexture.gTexture.handle));
	}
	...
	private void SetPerFrameImGuiData(float deltaSeconds)
	{
		...
		if (resolution is { x: > 0, y: > 0 })
			io.DisplayFramebufferScale = new Vector2((float)view.FramebufferSize.X / resolution.x, (float)view.FramebufferSize.Y / resolution.y);
