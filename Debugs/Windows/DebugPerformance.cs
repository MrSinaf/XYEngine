...
internal class DebugPerformance : DebugWindow
{
	...
	public override string menuItem => "Tools";
	public override Vector2 size => new (400, 200);
	public override ImGuiWindowFlags flags => ImGuiWindowFlags.AlwaysAutoResize;
	...
	public override void Render()
	{
		...
		ImGui.PlotLines("##fps", ref fpsHistory[0], fpsHistory.Length, historyIndex,
						$"{(count > 0 ? fpsHistory[(historyIndex - 1 + fpsHistory.Length) % fpsHistory.Length] : 0):F0} FPS",
						0, maxFps * 1.2f, new Vector2(ImGui.GetContentRegionAvail().x, 60));
		ImGui.PlotLines("##frameTime", ref frameTimeHistory[0], frameTimeHistory.Length, historyIndex,
						$"{(count > 0 ? frameTimeHistory[(historyIndex - 1 + frameTimeHistory.Length) % frameTimeHistory.Length] : 0):F2} ms",
		...
		ImGui.PlotLines("##memory", ref memoryHistory[0], memoryHistory.Length, historyIndex,
						$"{(count > 0 ? memoryHistory[(historyIndex - 1 + memoryHistory.Length) % memoryHistory.Length] : 0):F2} MB",
						0, maxMem * 1.2f, new Vector2(ImGui.GetContentRegionAvail().x, 60));
		
		if (ImGui.BeginTable("stats_table", 4, ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg | ImGuiTableFlags.SizingFixedSame))
		{
			...
			ImGui.Separator();
			ImGui.Spacing();
			XYDebug.ShowProperty("Runtime", TimeSpan.FromSeconds(Time.total).ToString(@"hh\:mm\:ss"));
		}
	}
}