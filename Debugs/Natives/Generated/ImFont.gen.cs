...
public unsafe partial struct ImFontPtr
{
	...
	public ImVector<float> IndexAdvanceX => new (NativePtr->IndexAdvanceX);
	...
	public ImVector<ushort> IndexLookup => new (NativePtr->IndexLookup);
	public ImPtrVector<ImFontGlyphPtr> Glyphs => new (NativePtr->Glyphs, Unsafe.SizeOf<ImFontGlyph>());
	public ImFontGlyphPtr FallbackGlyph => new (NativePtr->FallbackGlyph);
	public ImFontAtlasPtr ContainerAtlas => new (NativePtr->ContainerAtlas);
	public ImFontConfigPtr ConfigData => new (NativePtr->ConfigData);
	...
	public RangeAccessor<byte> Used4kPagesMap => new (NativePtr->Used4kPagesMap, 2);
	...
	public Vector2 CalcTextSizeA(float size, float max_width, float wrap_width, ReadOnlySpan<char> text_begin)
	{
		...
		if (text_begin_byteCount > Util.STACK_ALLOCATION_SIZE_LIMIT)
		{
		...
		var native_text_begin_offset = Util.GetUtf8(text_begin, native_text_begin, text_begin_byteCount);
	...
	public Vector2 CalcTextSizeA(float size, float max_width, float wrap_width, ReadOnlySpan<char> text_begin, ref byte* remaining)
	{
		...
		{
			ImGuiNative.ImFont_CalcTextSizeA(&__retval, (ImFont*)NativePtr, size, max_width, wrap_width, native_text_begin, native_text_begin + text_begin_byteCount,
											 native_remaining);
			if (text_begin_byteCount > Util.STACK_ALLOCATION_SIZE_LIMIT)
			{
				Util.Free(native_text_begin);
	...
	public string CalcWordWrapPositionA(float scale, ReadOnlySpan<char> text, float wrap_width)
	{
		...
		if (!text.IsEmpty)
		{
			...
			if (text_byteCount > Util.STACK_ALLOCATION_SIZE_LIMIT)
			{
		...
		var ret = ImGuiNative.ImFont_CalcWordWrapPositionA((ImFont*)NativePtr, scale, native_text, native_text + text_byteCount, wrap_width);
		if (text_byteCount > Util.STACK_ALLOCATION_SIZE_LIMIT)
		{
	...
	public void RenderText(ImDrawListPtr draw_list, float size, Vector2 pos, uint col, Vector4 clip_rect, ReadOnlySpan<char> text_begin)
	{
		...
		ImGuiNative.ImFont_RenderText((ImFont*)NativePtr, native_draw_list, size, pos, col, clip_rect, native_text_begin, native_text_begin + text_begin_byteCount, wrap_width,
									  cpu_fine_clip);
		if (text_begin_byteCount > Util.STACK_ALLOCATION_SIZE_LIMIT)
		{
			Util.Free(native_text_begin);
