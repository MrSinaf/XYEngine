...
public struct Vector2(float x, float y) : IEquatable<Vector2>
{
    ...
    public static Vector2 Lerp(Vector2 start, Vector2 end, float time)
    {
        ...
        return new Vector2(start.x + (end.x - start.x) * time, start.y + (end.y - start.y) * time);
    }

    public Vector2Int ToVector2Int(NumberOperation operation = NumberOperation.Round)
    {
        return operation switch
        {
            NumberOperation.Round => new Vector2Int((int)MathF.Round(x), (int)MathF.Round(y)),
            NumberOperation.Floor => new Vector2Int((int)MathF.Floor(x), (int)MathF.Floor(y)),
            NumberOperation.Ceiling => new Vector2Int((int)MathF.Ceiling(x), (int)MathF.Ceiling(y)),
